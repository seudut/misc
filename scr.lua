...
do
    local foo_proto = Proto("MSTR", "Cisco Multistream - P2P Call")
    
    local  foo_app_fb_id = ProtoField.string("foo.appid", "Application Feedback Identifier", base.NONE)
    local foo_type = ProtoField.uint16( "foo.type","Message Type",  base.DEC ,nil, 0xFFF0)
    local foo_ver = ProtoField.uint16( "foo.ver", "Version",   base.DEC,  nil,0x000F)
    local foo_seq = ProtoField.uint16("foo.seq", "Sequence Number", base.DEC, nil)
    foo_proto.fields = { foo_app_fb_id, foo_type, foo_ver, foo_seq,     }
...
--    local scr_pt_mmfs = ProtoField.uint16( "scr.ptlen","Length",  base.DEC ,nil)
    ...
    function foo_proto.dissector(tvb, pinfo, treeitem)
        ...
        --  Multistream Header
        foo_tree:add(foo_app_fb_id, tvb:range(offset, 4));  offset = offset + 4;        
        foo_tree:add(foo_type, tvb:range(offset,2))
        foo_tree:add(foo_ver, tvb:range(offset, 2));        offset = offset + 2;
        foo_tree:add(foo_seq, tvb:range(offset, 2));        offset = offset + 2;
        ...
        if msg_type == 1 then
            ...
            --- SCR
            local scr_tree = foo_tree:add("Sub-sesseion Channel Request", tvb:range(offset))
    ...
    --
        elseif msg_type == 2 then
            ...
            --- SCA
            local sca_tree = foo_tree:add("Sub-session Channel Announce")
        ...
        local foo_content_len = tvb_len-offset
--/        foo_tree:add(foo_seq, tvb:range(offset, foo_content_len))
        offset = offset+foo_content_len
        
    end
    local tcp_port_table = DissectorTable.get("rtcp.psfb.fmt")
    tcp_port_table:add(15, foo_proto)
end
